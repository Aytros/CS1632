...
public class World {
    ...
    private int getNumNeighbors(Cell[][] world, int x, int y) {
		...
		if (leftX == -1) { leftX = size - 1; }										//HOTSPOT HERE!!  original code contained an unnecessary
    	if (rightX == -1) { rightX = size - 1; }									//and extraneous for loop with 10000 iterations.
    	if (upY == -1) { upY = size - 1; }											//Once loop was removed, there was immediate performance boost
